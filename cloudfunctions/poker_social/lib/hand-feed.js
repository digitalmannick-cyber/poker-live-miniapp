...
const RECENT_FRIEND_SHARE_LIMIT = 3
const RECENT_FRIEND_SHARE_SCAN_LIMIT = 100
...
async function toRecentFriendShareItem(repository, viewerId, share, friendship) {
  const handId = text(share && share.source && share.source.handId)
  const hand = handId && await pointRead(repository, 'getSourceHandById', 'hands', handId)
  if (!exactSourceMatch(share, hand) || !canReadShare(viewerId, share, friendship)) return null
  let snapshot
  try {
    snapshot = copyHandSnapshot(share.snapshot)
  } catch (error) {
  ...
  const createdAt = Number(share.createdAt)
...
function createHandFeedHandlers(repository, options) {
  ...
  return {
    ...
    async list_friend_recent_shares(event, actor) {
      const viewer = await resolveViewer(repository, actor)
      const viewerId = text(viewer._id)
      const friendUserId = typeof (event && event.friendUserId) === 'string' ? text(event.friendUserId) : ''
      if (!friendUserId || friendUserId.length > MAX_SHARE_ID_LENGTH || friendUserId === viewerId) {
        throw socialError('FORBIDDEN', 'not allowed')
      }
      const friendship = await pointRead(repository, 'getFriendshipById', 'social_friendships', getPairId(viewerId, friendUserId))
      if (!friendship || friendship.status !== 'accepted' ||
        ![friendship.userA, friendship.userB].includes(viewerId) || ![friendship.userA, friendship.userB].includes(friendUserId)) {
        throw socialError('FORBIDDEN', 'not allowed')
      }
      const friend = await pointRead(repository, 'getSocialUserById', 'social_users', friendUserId)
      if (lifecycleOf(friend) !== SOCIAL_LIFECYCLE.ACTIVE) throw socialError('FORBIDDEN', 'not allowed')

      const items = []
      let cursor = null
      let scanned = 0
      while (items.length < RECENT_FRIEND_SHARE_LIMIT && scanned < RECENT_FRIEND_SHARE_SCAN_LIMIT) {
        const batch = await repository.listSelfShareCandidates(friendUserId, { cursor, limit: STREAM_BATCH_SIZE })
        if (!Array.isArray(batch)) throw new Error('friend share query unavailable')
        if (batch.some((row, index) => !row || !text(row._id) || !Number.isSafeInteger(row.createdAt) || row.createdAt <= 0 ||
          !isAfterCursor(row, cursor) || index > 0 && compareTuple(batch[index - 1], row) > 0)) {
          throw new Error('friend share order unavailable')
        }
        scanned += batch.length
        for (const share of batch) {
          const item = await toRecentFriendShareItem(repository, viewerId, share, friendship)
          if (item) items.push(item)
          if (items.length >= RECENT_FRIEND_SHARE_LIMIT) break
        }
        if (batch.length < STREAM_BATCH_SIZE) break
        const last = batch[batch.length - 1]
        cursor = { createdAt: Number(last.createdAt), id: text(last._id) }
      }
      return { items }
    }
  }
...
module.exports = {
  ...
  RECENT_FRIEND_SHARE_LIMIT,
  RECENT_FRIEND_SHARE_SCAN_LIMIT,
  getLikeId,
  canReadShare,
