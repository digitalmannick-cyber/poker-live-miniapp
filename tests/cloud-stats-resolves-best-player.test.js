...
test('sync_stats resolves the best owner account when local playerId is missing or stale', () => {
  ...
  assert.match(syncStatsSource, /recoveryCandidates\s*=\s*recoveryCandidates\s*\|\|\s*await listFencedRecoveryCandidates\(ownerOpenId\)/)
  assert.doesNotMatch(syncStatsSource, /await listRecoveryCandidates\(ownerOpenId\)/)
  ...
  assert.doesNotMatch(missingPlayerBranch, /list(?:Fenced)?RecoveryCandidates/)
})
