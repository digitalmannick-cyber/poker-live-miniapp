...
test('every planned index is verified with a hinted read before function staging', () => {
  ...
  assert.match(source, /hint = \$hint/)
  assert.match(source, /\$hint\[\[string\]\$field\.name\] = \[int\]\$field\.direction/)
  ...
  assert.match(source, /\$resolvedIndexNameByCanonical\.ContainsKey/)
})
...
test('function convergence uses the remote code digest and a staged startup smoke', () => {
  ...
  assert.match(source, /function Invoke-TcbJsonReadWithRetry/)
