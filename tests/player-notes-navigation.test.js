...
const playerNotesWxml = fs.readFileSync(path.join(__dirname, '..', 'pages', 'player-notes', 'player-notes.wxml'), 'utf8')
...
const friendHubWxss = fs.readFileSync(path.join(__dirname, '..', 'components', 'friend-hub', 'friend-hub.wxss'), 'utf8')
...
assert.match(playerNotesWxml, /wx:if="\{\{playerSection === 'friends'\}\}" class="player-notes-context-action player-notes-invite"[\s\S]*邀请好友/, 'friend invitation should only appear in the friends context')
assert.match(playerNotesWxml, /wx:if="\{\{playerSection === 'library'\}\}" class="player-notes-context-action player-notes-create"[\s\S]*新增玩家/, 'new player action should only appear in the player library context')
assert.doesNotMatch(playerNotesWxml, /player-notes-add-plus[\s\S]*>友</, 'the ambiguous friend glyph action should be removed')
assert.match(friendHubWxss, /\.friend-subnav-item\.active::after/, 'friend subsections should use a lower-weight underline treatment')
assert.doesNotMatch(friendHubWxss, /\.friend-subnav-item\.active\s*\{[^}]*background:\s*rgba\(230,0,18/s, 'friend subsection navigation should not duplicate the filled top-level tabs')
