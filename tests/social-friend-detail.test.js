...
test('friend detail keeps local player-note editing separate from remote social data', () => {
  ...
  assert.match(detailJs, /socialService\.listFriendRecentShares/)
  ...
  assert.match(detailWxml, /最近发布的手牌/)
  assert.match(detailWxml, /openRecentFriendShare/)
})
...
test('friend mode loads remote data, ensures the local note, and only local edits call updatePlayerNote', async () => {
  const { definition, restore, calls } = loadDetailPage({
    ...
    recentShares: [{
      shareId: 'share_recent', scope: 'friends', scopeLabel: '全部好友', createdAt: 1710000001000,
      likeCount: 2, commentCount: 1,
      summary: { heroCards: ['As', 'Ks'], board: { flop: ['Ah', '9s', '4d'], turn: [], river: [] }, potBb: 8.5, actionCount: 3 }
    }]
  })
  try {
    ...
    assert.equal(instance.data.recentFriendShares[0].shareId, 'share_recent')
    assert.equal(instance.data.recentFriendShares[0].potBbLabel, '8.5 BB')
    instance.openRecentFriendShare({ currentTarget: { dataset: { shareId: 'share_recent' } } })
    assert.deepEqual(calls.navigateTo, [{ url: '/pages/social-hand-detail/social-hand-detail?shareId=share_recent' }])
...
test('a recent-share outage does not make the core friend profile unavailable', async () => {
  const loaded = loadDetailPage({
    localNote: { _id: 'note_b', sourceKind: 'friend', linkedFriendUserId: 'su_b', name: '本地备注', type: '常客', leakTags: [], note: '', battleHandIds: [] },
    remote: friendDto(),
    recentSharesError: new Error('recent shares unavailable')
  })
  try {
    const instance = createInstance(loaded.definition)
    await instance.onLoad({ friendUserId: 'su_b' })
    assert.equal(instance.data.detailState, 'ready')
    assert.equal(instance.data.friend.socialUserId, 'su_b')
    assert.deepEqual(instance.data.recentFriendShares, [])
  } finally { loaded.restore() }
})
...
function loadDetailPage(options) {
  let definition = null
  const calls = { updatePlayerNote: [], removeFriend: [], detachFriendPlayerNote: [], switchTab: [], navigateTo: [], detachError: options.detachError, getFriendDetail: 0, listFriendRecentShares: 0, toasts: [] }
  ...
  Module._load = function load(request, parent, isMain) {
    if (parent && /pages[\\/]player-note-detail[\\/]player-note-detail\.js$/.test(parent.filename || '')) {
      ...
      if (request === '../../services/social-service') return {
        ...
        async listFriendRecentShares() { calls.listFriendRecentShares += 1; if (options.recentSharesError) throw options.recentSharesError; return { items: options.recentShares || [] } },
  ...
  global.wx = { showToast(input) { calls.toasts.push(input) }, switchTab(input) { calls.switchTab.push(input) }, navigateTo(input) { calls.navigateTo.push(input) }, navigateBack() {}, showModal(input) { input.success({ confirm: true }) } }
