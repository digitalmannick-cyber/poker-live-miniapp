...
function setup(seed, options) {
  ...
  assert.equal(typeof handlers.list_friend_recent_shares, 'function')
...
test('friend recent shares are server-authorized, newest-first, limited, and DTO-safe', async () => {
  const friendId = 'su_friend'
  const otherId = 'su_other'
  const rows = [
    share('square-new', friendId, 'square', 7000),
    share('selected-other', friendId, 'selected', 6500, { targetUserIds: [otherId] }),
    share('friends-mid', friendId, 'friends', 6000),
    share('selected-me', friendId, 'selected', 5000, { targetUserIds: [VIEWER_ID] }),
    share('square-old', friendId, 'square', 4000),
    share('withdrawn', friendId, 'square', 8000, { status: 'withdrawn' })
  ]
  const ctx = setup(baseSeed({
    social_users: [user(VIEWER_ID), user(friendId), user(otherId)],
    social_friendships: [accepted(VIEWER_ID, friendId)],
    social_hand_shares: rows,
    social_likes: [],
    hands: rows.map(row => sourceHand(row))
  }))

  const result = await ctx.handlers.list_friend_recent_shares({ friendUserId: friendId }, ACTOR)
  assert.deepEqual(result.items.map(item => item.shareId), ['square-new', 'friends-mid', 'selected-me'])
  assert.equal(result.items.length, 3)
  assert.deepEqual(Object.keys(result.items[0]).sort(), [
    'commentCount', 'createdAt', 'likeCount', 'scope', 'scopeLabel', 'shareId', 'summary'
  ])
  assert.equal(/ownerOpenId|privatePlayerId|source|targetUserIds|note|leakTags|profit|sessionId/.test(JSON.stringify(result)), false)
})

test('friend recent shares reject non-friends and stale source hands without disclosure', async () => {
  const friendId = 'su_friend'
  const visible = share('visible', friendId, 'square', 3000)
  const stale = share('stale', friendId, 'square', 4000)
  const nonFriend = setup(baseSeed({
    social_users: [user(VIEWER_ID), user(friendId)],
    social_friendships: [],
    social_hand_shares: [visible],
    social_likes: [],
    hands: [sourceHand(visible)]
  }))
  await expectCode(nonFriend.handlers.list_friend_recent_shares({ friendUserId: friendId }, ACTOR), 'FORBIDDEN')

  const acceptedFriend = setup(baseSeed({
    social_users: [user(VIEWER_ID), user(friendId)],
    social_friendships: [accepted(VIEWER_ID, friendId)],
    social_hand_shares: [stale, visible],
    social_likes: [],
    hands: [sourceHand(visible)]
  }))
  const result = await acceptedFriend.handlers.list_friend_recent_shares({ friendUserId: friendId }, ACTOR)
  assert.deepEqual(result.items.map(item => item.shareId), ['visible'])
})
