...
7. 第一版对广场文字评论与回复执行服务端自动检测，并实现服务端 OpenID 白名单管理员的评论软删除与操作审计；逐条举报、屏蔽、聊天和微信订阅消息暂不实现。
...
- 更广范围的自动内容检测、逐条举报、完整审核工作台与申诉：广场文字评论已在写入前同步检测；后续能力沿用同一云端鉴权边界，不改变分享快照模型。管理员紧急删除评论已经属于第一版能力。
