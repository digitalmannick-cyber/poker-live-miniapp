...
- 集成分支整项目串行回归（排除依赖未纳入 Git 的本机 `project.private.config.json` 环境测试）为 `977/977`；该环境测试由真实微信自动预览结果替代验证。
- 真实 CloudBase 部署、多账号真机矩阵和开发版上传仍是外部环境步骤，必须按部署清单取得授权并配置仅存于云端的密钥后执行，不得把它们误记为本地代码已完成。
