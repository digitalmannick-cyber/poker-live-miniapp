...
分享内部 source 固定为 `{ ownerOpenId, playerId, handId }`。每条去重后的候选在输出前必须：

1. 用 `source.handId` 对 `hands` 做 document point-read；禁止 `where + in` 批量替代，禁止读 slot、缓存或快照来推断源仍存在。
2. 精确比较 `hand._id === source.handId`、`hand.ownerOpenId === source.ownerOpenId`、规范化后的 `hand.playerId === source.playerId`；任一缺失或不一致即过滤。
